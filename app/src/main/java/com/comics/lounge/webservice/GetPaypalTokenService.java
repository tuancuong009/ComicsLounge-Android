...
public class GetPaypalTokenService extends CoreWebService implements Runnable {
    public static final String SERVICE_NAME = "GetPaypalTokenService";
    ...
    public GetPaypalTokenService(String url, ServiceCallback serviceCallback) {
        super(url);
    ...
    private void extractCategory(String lastResponse) {
        try {
            ...
            if (!outerObj.isNull("status") && outerObj.getString("status").equals("Success")) {
                if (!outerObj.isNull("token")) {
                    setPaypalToken(outerObj.getString("token"));
                }
            }
