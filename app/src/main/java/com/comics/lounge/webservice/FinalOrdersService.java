...
public class FinalOrdersService extends CoreWebService implements Runnable {
    public static final String SERVICE_NAME = "FinalOrdersService";
    ...
    public FinalOrdersService(String url, ServiceCallback serviceCallback) {
        super(url);
    ...
    public void run() {
        ...
        try {
            ...
            getResponse();
            Log.e("Print_Scrooll", getLastResponse());
    ...
    private void extractCategory(String lastResponse) {
        if (lastResponse != null && !lastResponse.equals("")) {
            try {
                ...
                if (outerObj.has("status") && !outerObj.isNull("status")) {
