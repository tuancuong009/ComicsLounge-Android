...
public class ConfirmTickertService extends CoreWebService implements Runnable {
    public static final String SERVICE_NAME = "ConfirmTickertService";
    ...
    private List<ConfirmTicket> confirmTicketList = null;

    public ConfirmTickertService(String url, ServiceCallback serviceCallback) {
        super(url);
        this.serviceCallback = serviceCallback;
        confirmTicketList = new LinkedList<ConfirmTicket>();
    }
    ...
    public void run() {
        ...
        try {
            ...
            getResponse();
            confirmTicketList.addAll(gettingApiResponse(getLastResponse()));
    ...
    private List<ConfirmTicket> gettingApiResponse(String lastResponse) {
        List<ConfirmTicket> events = new LinkedList<ConfirmTicket>();
        try {
            ...
            if (!outerObj.isNull("orders")) {
                ...
                for (int i = 0; i < userObj.length(); i++) {
                    ...
                    confirmTicket.setOrderId(eventObj.getInt("order_id"));
                    confirmTicket.setDownloadLink(eventObj.getString("download_link"));
                    ...
                    if (itemArray.length() > 0) {
                        for (int j = 0; j < itemArray.length(); j++) {
                            ...
                            bookingHistory.setPrice(itemObj.has("price") && !itemObj.isNull("price") ? itemObj.getDouble("price") : 0.0);
                            ...
                            confirmTicket.addBookingHisObj(bookingHistory);
                        }
                        events.add(confirmTicket);
                    }
    ...
    public List<ConfirmTicket> getConfirmTicketList() {
        return confirmTicketList;
    }
